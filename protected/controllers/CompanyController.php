...
class CompanyController extends Controller {
	...
	public function accessRules() {
		...
		'actions' => array('index', 'view', 'create', 'update', 'admin', 'delete'), 'users' => User::usernamesByRole((User::ADMIN | User::OWNER), User::PERM_ORDER), ), array('deny', // deny all users
		'users' => array('*'), ), );
	}
	...
	public function uploadFile($model, $attribute, $prevValue="") {
	...
	public function actionUpdate($id) {
		...
		if (isset($_POST['Company'])) {
			$prevMap=$model->map;
			$prevLogotype=$model->logotype;
			
			$model -> attributes = $_POST['Company'];
			$model -> logotype = $this -> uploadFile($model, 'logotype', $prevLogotype);
			$model -> map = $this -> uploadFile($model, 'map', $prevMap);
			if ($model -> save())
				$this -> redirect(array('update', 'id' => $model -> idCompany));
		}
